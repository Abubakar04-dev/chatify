        ...
        <div class="row g-3">
            <div class="col-12 col-md-4">
                <div class="input-group">
                    ...
                    <input type="text" id="search-input" class="form-control" placeholder="Search users..." value="{{ request('search') }}">
                </div>
            </div>
            <div class="col-6 col-md-3">
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <img src="{{ $user->avatar ? asset('storage/users-avatar/' . $user->avatar) : asset('images/avatar.png') }}" class="user-avatar me-2">
                                <div>
                        ...
                        <td>
                            @if($user->isAdmin())
                            <span class="badge badge-role bg-info">Admin</span>
                            @else
                            <span class="badge badge-role bg-secondary">User</span>
                            @endif
                        </td>
                        <td>
                            <span class="badge badge-status status-badge bg-{{ $user->status == 'active' ? 'success' : ($user->status == 'inactive' ? 'warning' : 'danger') }}">
                                {{ ucfirst($user->status) }}
                            </span>
                        </td>
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                ...
                                <button class="btn btn-info toggle-status-btn" data-user-id="{{ $user->id }}" title="Toggle Status">
                                    <i class="fas fa-sync-alt"></i>
                                </button>
                                @endif
                                @if(auth()->user()->isSuperAdmin() && $user->id !== auth()->id())
                                <button class="btn btn-danger delete-user-btn" data-user-id="{{ $user->id }}" data-user-name="{{ $user->name }}" title="Delete">
...
<script>
    $(document).ready(function() {
        ...
        function filterUsers() {
            ...
            $('#users-table-container').html('<div class="text-center py-5"><i class="fas fa-spinner fa-spin fa-2x text-primary"></i></div>');
            ...
            $.ajax({
                ...
                error: function() {
                    $('#users-table-container').html('<div class="text-center py-5 text-danger">Failed to load users</div>');
                }
            });
        }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        let statusText = response.status.charAt(0).toUpperCase() + response.status.slice(1);
